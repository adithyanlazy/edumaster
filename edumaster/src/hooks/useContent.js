      ...
      .then((data) => {
        ...
        for (const key of Object.keys(data)) {
          if (defaultContent[key]) {
            merged[key] = Array.isArray(defaultContent[key]) ? data[key] : { ...defaultContent[key], ...data[key] }
          }
        }
        setContent(merged)
        Object.assign(contentCache, merged)
      })
      .catch(() => {})
...
export default function useContent(section) {
  const [data, setData] = useState(() => {
    if (contentCache[section]) return contentCache[section]
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/content/${section}`)
      ...
      .then((remote) => {
        ...
        const merged = Array.isArray(fallback) ? remote : { ...fallback, ...remote }
        contentCache[section] = merged
        setData(merged)
      })
      .catch(() => {
